...
def getcalender(course,server_status=0):
    #global dtstart_time, dtend_time
    ...
    for one_course in course:
        if one_course['kcmc'] == '大学物理实验A1':
            ...
            if course_period_start == 5:
                dtstart_time = time(13,30)
                dtend_time = time(15,10)
            else:
                dtstart_time = time(15,30)
                dtend_time = time(17,10)
            #else:
            #    exit(404)
